...
#    app.add_handler(incoming_size_checker_handler)
    ...
    incoming_g_clear_handler = MessageHandler(
        g_clearme,
        filters=Filters.command([f"{CLEAR@Leeech2filebot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_g_clear_handler)
    #
    incoming_youtube_dl_handler = MessageHandler(
        incoming_youtube_dl_f,
        filters=Filters.command([f"{YTDL@Leeech2filebot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_youtube_dl_handler)
    #
    incoming_youtube_playlist_dl_handler = MessageHandler(
        g_yt_playlist,
        filters=Filters.command([f"{PYTDL@Leeech2filebot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_youtube_playlist_dl_handler)
    #
    status_message_handler = MessageHandler(
        status_message_f,
        filters=Filters.command([f"{STATUS@Leeech2filebot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(status_message_handler)
    #
    cancel_message_handler = MessageHandler(
        cancel_message_f,
        filters=Filters.command([f"{CANCEL@Leeech2filebot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command([f"{SAVETHUMBNAIL@Leeech2filebot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=Filters.command([f"{CLEARTHUMBNAIL@Leeech2filebot}"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
