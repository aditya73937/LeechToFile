...
if __name__ == "__main__" :
    ...
    incoming_message_handler = MessageHandler(
        incoming_message_f,
        filters=Filters.command(["Leech@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
...
#    app.add_handler(incoming_telegram_download_handler)
    ...
    incoming_purge_message_handler = MessageHandler(
        ...
        filters=Filters.command(["purge@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
...
#    app.add_handler(incoming_size_checker_handler)
    ...
    incoming_g_clear_handler = MessageHandler(
        g_clearme,
        filters=Filters.command(["CLEAR@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_g_clear_handler)
    #
    incoming_youtube_dl_handler = MessageHandler(
        incoming_youtube_dl_f,
        filters=Filters.command(["YTDL@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_youtube_dl_handler)
    #
    incoming_youtube_playlist_dl_handler = MessageHandler(
        g_yt_playlist,
        filters=Filters.command(["PYTDL@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_youtube_playlist_dl_handler)
    #
    status_message_handler = MessageHandler(
        status_message_f,
        filters=Filters.command(["STATUS@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(status_message_handler)
    #
    cancel_message_handler = MessageHandler(
        cancel_message_f,
        filters=Filters.command(["CANCEL@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(cancel_message_handler)
    #
    exec_message_handler = MessageHandler(
        exec_message_f,
        filters=Filters.command(["exec@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    eval_message_handler = MessageHandler(
        eval_message_f,
        filters=Filters.command(["eval@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    rename_message_handler = MessageHandler(
        rename_message_f,
        filters=Filters.command(["rename@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(rename_message_handler)
    #
    upload_document_handler = MessageHandler(
        upload_document_f,
        filters=Filters.command(["upload@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(upload_document_handler)

    help_text_handler = MessageHandler(
        help_message_f,
        filters=Filters.command(["help@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command(["SAVETHUMBNAIL@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=Filters.command(["CLEARTHUMBNAIL@Leeech2filebot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
